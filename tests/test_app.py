...
class StreamlitIntegrationTests(unittest.TestCase):
    def test_gemini_streamlit_secrets_are_forwarded(self) -> None:
        fake_secrets = {
        ...
        clean_environment = {
            ...
            if name not in app.STREAMLIT_SECRET_NAMES
        }

        with patch.dict(os.environ, clean_environment, clear=True), patch.object(
            app.st, "secrets", fake_secrets
        ):
            app.configure_runtime_from_streamlit_secrets()
            self.assertEqual(os.environ["LABMIND_PROVIDER"], "gemini")
